...
const Login = ({ onBack }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (isSignUp) {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_USUARIOS}?username=${email}`);
        if (res.data.length > 0) {
          setError("El usuario ya existe");
          return;
        }
        await axios.post(import.meta.env.VITE_API_USUARIOS, { username: email, password, rol: 'usuario' });
      ...
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_USUARIOS}?username=${email}&password=${password}`);
